...
pm.test("Verify returned id & description", function () {
    ...
    pm.expect(jsonData.description).to.eql("Test RMNCAH")
});
...
pm.test("Verify returned visualizations", function () {
    ...
    pm.expect(vis.name).to.eql('RMNCAH - Trends in coverage of interventions for maternal and newborn (%)');
})
...
postman.setNextRequest("Update a job");
